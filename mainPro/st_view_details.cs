...
namespace mainPro
{
    public partial class st_view_details : Form
    {
        ...
        private void student_load()
        {
            ...
            da.Fill(d_set);
            conn.Close();
            DataTable d_tb = new DataTable();
            d_tb = d_set.Tables[0];
            DataTable d_tb2 = new DataTable();
                d_tb2.Columns.Add("text");
            d_tb2.Columns.Add("value");
            int i;
           
            
                foreach(DataRow x in d_tb.Rows)
                {
                    d_tb2.Rows.Add(x[0].ToString() + "---" + x[1].ToString() + " " + x[2].ToString(), x[0].ToString());
                }
            ...
            if (comboBox2.Items.Count >= 1)
            {
                ...
                if (d_view!=null)
                {
                    ...
                    try
                    {
                        ...
                        cmd.CommandText = "use " + comboBox1.SelectedValue + ";select * from s_info where s_id = '" + d_view["value"].ToString() + "';";
                        conn.Open();
                        cmd.ExecuteNonQuery();
                    }
                    catch (Exception ex)
        ...
        private void submit_Click(object sender, EventArgs e)
        {
            ...
            SqlCommand cmd = new SqlCommand("use " + dhg["da_name"].ToString() + "; update s_info set f_name=@p1,l_name=@p2,pic=@p3,gen=@p4,address=@p5,pin=@p6,country=@p7,c_no=@p8,email=@p9,dob=@p10,other_info=@p11 where s_id=@p12;",con);
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if(MessageBox.Show("do you really wnt to delete the record","attantion",MessageBoxButtons.YesNo)==DialogResult.Yes)
            {
                ...
                cmd.CommandType = CommandType.Text;
                DataRowView d_view = (DataRowView)comboBox2.SelectedItem;
                cmd.CommandText = "use " + comboBox1.SelectedValue + ";delete s_info where s_id = '" + d_view["value"].ToString() + "';";
                conn.Open();
                cmd.ExecuteNonQuery();
                cmd.CommandText = "use " + comboBox1.SelectedValue + ";drop table  _" + d_view["value"].ToString() + ";";
                cmd.ExecuteNonQuery();
                conn.Close();
